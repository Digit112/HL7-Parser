...
class HL7ParsedMessage extends HL7ParsedEntity {
	constructor(grammar, body) {
		...
		let segment_bodies = body.split(/(?:\r\n|\r|\n)/) // Split on CR (as per specification requirements), LF, or CRLF
		...
		if (header_fields.length < 9) {
			...
			message_type_id = `${cm_msg_message_type} ${cm_msg_event_type}`.trim()
		}
