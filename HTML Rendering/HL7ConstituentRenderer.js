class HL7ConstituentRenderer {
	...
	constructor(parsed_constituent, constituent_description_div, entity_description_div) {
		if (!(parsed_constituent instanceof HL7ParsedConstituent))
			throw new Error("Passed parsed entity must be an HL7ParsedConstituent.")
	...
	render(level) {
		...
		if (this.constituent_type.get_metatype() == "PRIMITIVE") {
			...
			if (this.parsed_constituent.body.length > 0) {
			...
			for (let rep_i = 0; rep_i < this.parsed_constituent.repetitions.length; rep_i++) {
				...
				for (let comp_i = 0; comp_i < repetition.length; comp_i++) {
					...
					let component_renderer = new HL7ConstituentRenderer(component, this.constituent_description_div, this.entity_description_div)
